...
int main(int argc, char** argv)
{
	...
	// while((data=faster_file_reader_next(reader))!=NULL&&brkg_condition==false) 
	{
		...
		if(isLabelX==1&&isLabelY==1)
		{
			...
			{
				t_last_refresh=fasterTime;
				ofstream gnuplot_file("./Gnuplot_data.gpt",ios::out);
				for(int ig=0;ig<N_STRIPS;ig++)
				{
					if(label==LabelX)
					{
						// gnuplot_file<<PreSFB.at(ig)<<" "<<PreSFBprim.at(ig)<<" "<<PostSFB.at(ig)<<" "<<PostSFBprim.at(ig)<<endl;
					...
					if(label==LabelY)
					{
						// gnuplot_file<<PreSFBprim.at(ig)<<" "<<PreSFB.at(ig)<<" "<<PostSFBprim.at(ig)<<" "<<PostSFB.at(ig)<<endl;
						// for(int k=0;k<N_STRIPS;k++)
						// {
						// 	if(sum_val>0.)
						// 		pixel=PostSFB.at(i)*PostSFBprim.at(k)/sum_val;
	...
	if(bkgnd_param==3)
	{
		ofstream bdf_file("Offset.txt",ios::out);
		for(int i=0;i<N_STRIPS;i++)
